...
class Bug(Base):
    ...
    priority = Column(Enum('urgent', 'high', 'medium', 'low'), default='medium', index=True)
...
class Requirement(Base):
    ...
    content = Column(Text)
    priority = Column(Enum('urgent', 'high', 'medium', 'low'), default='medium', index=True)
    status = Column(Enum('not_started', 'developing', 'testing', 'completed'), default='not_started', index=True)
    ...
    creator = relationship("User", foreign_keys=[created_by])
    children = relationship("Requirement", foreign_keys=[parent_id],
                            backref=backref("parent_req", remote_side=[id]))
...
class WorkTask(Base):
    ...
    content = Column(Text)
    priority = Column(Enum('urgent', 'high', 'medium', 'low'), default='medium', index=True)
    status = Column(Enum('not_started', 'in_progress', 'completed'), default='not_started', index=True)
    assignee_id = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
    created_by = Column(Integer, ForeignKey("users.id", ondelete="RESTRICT"), nullable=False, index=True)
    parent_id = Column(Integer, ForeignKey("work_tasks.id", ondelete="SET NULL"), nullable=True, index=True)
    start_date = Column(Date, nullable=True)
    due_date = Column(Date, nullable=True)
    created_at = Column(DateTime, default=datetime.now)
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now)

    project = relationship("Project", backref="work_tasks")
    sprint = relationship("Sprint", foreign_keys=[sprint_id])
    assignee = relationship("User", foreign_keys=[assignee_id])
    creator = relationship("User", foreign_keys=[created_by])
    children = relationship("WorkTask", foreign_keys=[parent_id],
                            backref=backref("parent_task", remote_side=[id]))


class TestCaseDirectory(Base):
    """用例目录表（独立持久化，不依赖占位用例）"""
    __tablename__ = "testcase_directories"

    id = Column(Integer, primary_key=True, index=True)
    project_id = Column(Integer, ForeignKey("projects.id", ondelete="CASCADE"), nullable=False, index=True)
    path = Column(String(500), nullable=False)   # 完整路径，如 "模块A/子模块B"
    name = Column(String(200), nullable=False)   # 目录名（最后一段）
    created_at = Column(DateTime, default=datetime.now)

    project = relationship("Project", backref="testcase_directories")
