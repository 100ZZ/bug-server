...
CREATE TABLE IF NOT EXISTS api_environments (
    ...
    base_url VARCHAR(255) NOT NULL COMMENT '配置信息（基础URL）',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='接口测试配置表';
...
CREATE TABLE IF NOT EXISTS requirements (
    ...
    created_by INT NOT NULL COMMENT '创建人ID',
    parent_id INT COMMENT '父需求ID（NULL表示顶级需求）',
...
CREATE TABLE IF NOT EXISTS work_tasks (
    ...
    assignee_id INT COMMENT '处理人ID',
    created_by INT NOT NULL COMMENT '创建人ID',
    parent_id INT COMMENT '父任务ID（NULL表示顶级任务）',
    start_date DATE COMMENT '开始日期',
    due_date DATE COMMENT '截止日期',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    FOREIGN KEY (assignee_id) REFERENCES users(id) ON DELETE SET NULL,
    FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE RESTRICT,
    FOREIGN KEY (parent_id) REFERENCES work_tasks(id) ON DELETE SET NULL,
    INDEX idx_project_id (project_id),
    INDEX idx_sprint_id (sprint_id),
    INDEX idx_assignee_id (assignee_id),
    INDEX idx_created_by (created_by),
