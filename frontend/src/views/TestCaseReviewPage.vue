  ...
  <div class="testcase-review-page">
    ...
    <el-card class="table-card">
      ...
      >
        ...
        <el-table-column label="迭代" min-width="150" align="center">
          <template #default="{ row }">
            {{ row.sprint?.name || '-' }}
          </template>
        </el-table-column>
        ...
        <el-table-column prop="start_date" label="发起时间" width="120" align="center">
          <template #default="{ row }">
      ...
      <div style="margin-top: 16px; text-align: right;">
        <el-pagination
          ...
          @size-change="() => { currentPage.value = 1; loadReviews() }"
        />
      </div>
    </el-card>
...
import { Plus, Search, EditPen, Delete, Document, DocumentChecked, Folder, FolderOpened, Finished } from '@element-plus/icons-vue'
...
/** 当前评审已关联的用例 ID（打开抽屉时设置），用于列表显示全部用例且确定时只提交新增的 */
const selectTestCasesExistingIds = ref<Set<number>>(new Set())
...
const loadReviews = async () => {
  ...
  try {
    ...
    const res = await reviewApi.getTestCaseReviews(params)
...
const loadSprintsForProject = async () => {
  ...
  try {
    const res = await sprintApi.getSprints({ project_id: formData.project_id, page: 1, page_size: 1000 })
...
const loadAvailableTestCases = async () => {
  ...
  try {
    ...
    availableTestCases.value = res.items.filter(tc => !existingTestCaseIds.has(tc.id))
  } catch (error: any) {
...
const getReviewItemStatusType = (status: string): string | undefined => {
...
const getPriorityTag = (priority: string): string | undefined => {
...
const getStatusType = (review: TestCaseReview | null): string | undefined => {
  ...
  const type = typeMap[status as keyof typeof typeMap]
  return type || undefined
}
  ...
  if (selectTestCasesSelectedDirectoryPath.value !== null) {
    filtered = filtered.filter(testcase => {
      ...
      return modulePath === selectTestCasesSelectedDirectoryPath.value ||
             modulePath.startsWith(selectTestCasesSelectedDirectoryPath.value + '/')
    })
  }
...
const restoreSelectedTestCases = () => {
  nextTick(() => {
    if (!selectTestCasesTableRef.value) return
    // ⚠️ 必须先快照 IDs！clearSelection() 会同步触发 selection-change([])，
    // handler 会清空 selectTestCasesSelectedTestCaseIds 中当前页的所有 ID，
    // 如果直接用响应式引用，后续 toggleRowSelection 循环里 has() 全为 false，什么都勾不上。
    const idsSnapshot = new Set(selectTestCasesSelectedTestCaseIds.value)
    selectTestCasesTableRef.value.clearSelection()
    selectTestCasesPaginatedTestCases.value.forEach((row: TestCase) => {
      if (idsSnapshot.has(row.id)) {
...
const handleSelectTestCases = async (row: TestCaseReview) => {
  ...
  selectTestCasesExistingIds.value = new Set()
  selectTestCasesCurrentPage.value = 1
  // 每次打开先清空列表与树，避免沿用上一次的数据导致筛选数量不一致
  selectTestCasesAllTestCases.value = []
  selectTestCasesAllTestCasesForTree.value = []
  selectTestCasesUnlinkedTestCases.value = []
  ...
  try {
    ...
    const res = await testcaseApi.getTestCases(params)
    const allTestCases = res.items ?? []
    // 若用户已切换到其他评审，不应用本次结果，避免数量错乱
    if (selectTestCasesReview.value?.id !== openingReviewId) return

    selectTestCasesAllTestCasesForTree.value = allTestCases

    const reviewItemsRaw = await reviewApi.getReviewItems(row.id)
    if (selectTestCasesReview.value?.id !== openingReviewId) return
    const reviewItems = Array.isArray(reviewItemsRaw) ? reviewItemsRaw : (reviewItemsRaw?.data ?? reviewItemsRaw?.items ?? [])
    // 兼容 testcase_id 或嵌套 testcase.id，确保 7 个保存后重开能显示 7 个已选
    const existingTestCaseIds = new Set(
      reviewItems
        .map((item: { testcase_id?: number; testcase?: { id: number } }) => item.testcase_id ?? item.testcase?.id)
        .filter((id: number | undefined): id is number => id != null)
    )
    selectTestCasesSelectedTestCaseIds.value = new Set(existingTestCaseIds)
    selectTestCasesExistingIds.value = new Set(existingTestCaseIds)
    selectTestCasesUnlinkedTestCases.value = allTestCases.filter(tc => !existingTestCaseIds.has(tc.id))
    selectTestCasesAllTestCases.value = allTestCases
    // 初始化完成后，主动恢复表格勾选状态
    restoreSelectedTestCases()
  } catch (error: any) {
    if (selectTestCasesReview.value?.id === openingReviewId) {
...
const handleConfirmSelectTestCases = async () => {
  ...
  try {
    // 只提交“本次新勾选”的用例（排除打开时已关联的），避免重复添加
    const selectedTestCases = selectTestCasesAllTestCases.value.filter(tc =>
      selectTestCasesSelectedTestCaseIds.value.has(tc.id) && !selectTestCasesExistingIds.value.has(tc.id)
    )
...
const reviewDrawerApprovedCount = computed(() => {
  return reviewDrawerReviewItems.value.filter(item => item.status === 'approved').length
})

// 进度条百分比 = 已评审（通过+不通过）/ 总数，反映评审完成进度
...
// 通过率 = 通过数 / 已评审数（已评审才有通过率意义）
const reviewDrawerPassRate = computed(() => {
  if (reviewDrawerReviewedCount.value === 0) return 0
  return Math.round((reviewDrawerApprovedCount.value / reviewDrawerReviewedCount.value) * 100)
})

// 进度条颜色根据通过率来决定，而非完成进度
const reviewDrawerProgressStatus = computed(() => {
  if (reviewDrawerReviewedCount.value === 0) return ''
  const passRate = reviewDrawerPassRate.value
...
const reviewDrawerProgressText = computed(() => {
  const reviewed = reviewDrawerReviewedCount.value
  const total = reviewDrawerTotalCount.value
  const approved = reviewDrawerApprovedCount.value
  if (total === 0) return '暂无用例'
  const passRate = reviewDrawerPassRate.value
  return `已评 ${reviewed}/${total}，通过 ${approved} 个（${passRate}%）`
})
