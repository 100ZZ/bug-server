    ...
    <el-card class="table-card">
      ...
      <div style="margin-top: 16px; text-align: right;">
        <el-pagination
          ...
          :total="totalScans"
          @current-change="loadScans"
          @size-change="() => { currentPage.value = 1; loadScans() }"
        />
      </div>
    </el-card>
...
const loadScans = async () => {
  ...
  try {
    ...
    const res = await codeScanApi.getCodeScans(params)
    scans.value = res.items
    totalScans.value = res.total
  } catch (error: any) {
...
const paginatedScans = computed(() => scans.value)

const cleanupScanListener = onProjectChanged(() => {
  if (hasProjectSelected.value && getCurrentProjectId.value) {
    filters.project_id = getCurrentProjectId.value
  }
  currentPage.value = 1
