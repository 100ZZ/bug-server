<template>
  <div class="testcases-page">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              ...
              <el-dropdown-item command="import" divided>
                ...
                <span style="margin-left: 4px;">导入用例</span>
              </el-dropdown-item>
              <el-dropdown-item
                command="batchDelete"
      ...
      <div class="testcases-main" :style="{ width: `calc(100% - ${sidebarWidth + 20}px)` }">
        ...
        <el-card class="testcases-list-card">
          <div style="flex: 1; min-height: 0; display: flex; flex-direction: column;">
            ...
            >
            <el-table-column type="selection" width="55" align="center" @click.stop />
            <el-table-column label="编号" width="160" align="center">
              <template #default="{ row }">
                {{ row.case_key }}
              </template>
            </el-table-column>
            <el-table-column prop="title" label="标题" min-width="350" align="center" show-overflow-tooltip />
            ...
            <div style="margin-top: 16px; text-align: right; flex-shrink: 0;">
              <el-pagination
                ...
                :total="total"
                @size-change="() => { currentPage.value = 1; fetchTestCases() }"
    ...
    >
      <template #header>
        <div class="dialog-header">
          <span class="dialog-title">批量导入用例</span>
          <span class="dialog-description">通过导出的 Excel 或 CSV 文件批量创建测试用例</span>
        </div>
      </template>

      <!-- 当前项目（置灰显示） -->
      <div class="import-project-row">
        <span class="import-project-label">导入到项目</span>
        <el-input
          :value="importCurrentProjectName"
          disabled
          style="flex: 1;"
        />
      </div>

      <div class="import-step" style="margin-top: 14px;">
        <div class="import-step-label">
          <span class="import-step-num">1</span>
          先通过"导出 Excel / CSV"获取文件，填写后导入
        </div>
        <el-alert type="info" :closable="false" style="margin-top: 8px;">
          <template #default>
            支持 Excel（.xlsx）和 CSV（.csv）。导入字段：<strong>标题</strong>（必填）、分组、等级（P0–P4）、前置条件、步骤N、预期结果N。其余列自动忽略。
          </template>
        </el-alert>
      </div>
...
import { Plus, Search, EditPen, Delete, Document, MagicStick, UploadFilled, Folder, FolderOpened, CopyDocument, ArrowDown, Download, Upload } from '@element-plus/icons-vue'
import * as testcaseApi from '@/api/testcases'
import type { TestCaseDirectory } from '@/api/testcases'
...
const treeTestcases = ref<TestCase[]>([])
// 持久化目录列表（从后端 testcase_directories 表加载）
const treeDirectories = ref<TestCaseDirectory[]>([])
const total = ref(0)
...
const importCurrentProjectName = computed(() => {
  const pid = getProjectId()
  if (!pid) return '（未选择项目，请先在顶部选择）'
  const p = projects.value.find((p: any) => p.id === pid)
...
const getProjectId = () => {
  if (hasProjectSelected.value && getCurrentProjectId.value) return getCurrentProjectId.value
...
const fetchTreeData = async () => {
  ...
  try {
    ...
    const [casesRes, dirsRes] = await Promise.all([
      testcaseApi.getTestCases(params),
      projectId ? testcaseApi.getTestCaseDirectories(projectId) : Promise.resolve([])
    ])
    treeTestcases.value = casesRes.items
    treeDirectories.value = Array.isArray(dirsRes) ? dirsRes : []
  } catch {
    // 目录树加载失败静默处理
  }
}
...
const fetchTestCases = async () => {
  ...
  try {
    ...
    if (selectedDirectoryPath.value !== null) params.module = selectedDirectoryPath.value
    if (testcaseSearchKeyword.value) params.search = testcaseSearchKeyword.value
...
const refreshAll = async () => {
  await Promise.all([fetchTreeData(), fetchTestCases()])
}

// 构建目录树（合并：用例 module 字段 + directories 表中的持久化目录）
const buildDirectoryTree = (testcases: TestCase[], directories: TestCaseDirectory[] = []): DirectoryTreeNode[] => {
  const directoryMap = new Map<string, { count: number, pathParts: string[] }>()

  const ensurePath = (modulePath: string, countIncrement = 0) => {
    ...
    pathParts.forEach((part, index) => {
      ...
      if (index === pathParts.length - 1) {
        directoryMap.get(currentPath)!.count += countIncrement
      }
    })
  }
  ...
  testcases.forEach(tc => {
    ...
    if (modulePath) ensurePath(modulePath, 1)
  })

  // directories 表贡献目录节点（保证空目录也出现，count 不额外叠加）
  directories.forEach(dir => ensurePath(dir.path, 0))
...
// 目录树数据（基于全量用例 module 字段 + 持久化目录表）
const directoryTreeData = computed(() => {
  return buildDirectoryTree(treeTestcases.value, treeDirectories.value)
})
...
const paginatedTestCases = computed(() => testcases.value)
...
// 创建目录的公共逻辑
const createDirectory = async (newPath: string, newDirectoryName: string) => {
  if (!hasProjectSelected.value || !getCurrentProjectId.value) {
    ElMessage.warning('请先选择项目')
    return
  }
  // 检查是否已存在（目录表或用例 module 中）
  const existsInDirs = treeDirectories.value.some(d => d.path === newPath || d.path.startsWith(`${newPath}/`))
  const existsInCases = treeTestcases.value.some(tc => {
    const m = tc.module || ''
    return m === newPath || m.startsWith(`${newPath}/`)
  })
  if (existsInDirs || existsInCases) {
    ElMessage.warning('该分组已存在')
    return
  }
  await testcaseApi.createTestCaseDirectory({
  ...
  await nextTick()
  if (directoryTreeRef.value) {
    directoryTreeRef.value.setCurrentKey(`dir-${newPath}`)
  }
}
...
const handleAddMainDirectory = async () => {
  try {
    const { value } = await ElMessageBox.prompt('输入主分组名称', '新建主分组', {
      ...
      inputValidator: (v) => {
        if (!v || !v.trim()) return '目录名称不能为空'
        if (v.includes('/')) return '目录名称不能包含"/"'
        return true
      }
    })
    if (value) await createDirectory(value.trim(), value.trim())
  } catch (error: any) {
    if (error !== 'cancel') {
      ElMessage.error(error.message || error.response?.data?.detail || '创建分组失败')
    }
  }
}
...
const handleAddSubDirectory = async (parentNode: DirectoryTreeNode) => {
  try {
    ...
    if (value) {
      ...
      await createDirectory(newPath, name)
    }
  } catch (error: any) {
    if (error !== 'cancel') {
      ElMessage.error(error.message || error.response?.data?.detail || '创建分组失败')
...
// 编辑目录（重命名分组）——后端统一处理子目录路径和用例 module 的更新
  ...
  try {
    const { value } = await ElMessageBox.prompt('输入新的分组名称', '重命名分组', {
      ...
      inputValidator: (v) => {
        ...
        if (v.includes('/')) return '分组名称不能包含"/"'
        return true
      }
    })
    if (value) {
      const newName = value.trim()
      if (newName === node.label) return
      const pathParts = node.path!.split('/')
      pathParts[pathParts.length - 1] = newName
      const newPath = pathParts.join('/')

      // 找到目录记录（可能是从 directories 表来的，也可能是从用例 module 推导出来的）
      const dirRecord = treeDirectories.value.find(d => d.path === node.path)
      if (dirRecord) {
        // 有目录记录：后端接口会同步子目录和用例 module
        await testcaseApi.updateTestCaseDirectory(dirRecord.id, { path: newPath, name: newName })
      } else {
        // 纯粹从用例 module 推导的目录：逐条更新用例 module
        const affected = treeTestcases.value.filter(tc => {
          const m = tc.module || ''
          return m === node.path || m.startsWith(`${node.path}/`)
        })
        const currentUser = getCurrentUser()
        for (const tc of affected) {
          if (!tc.id || !tc.module) continue
          const newModule = tc.module === node.path ? newPath : newPath + tc.module.slice(node.path!.length)
          await testcaseApi.updateTestCase(tc.id, { ...tc, module: newModule, updated_by: currentUser?.user_id })
        }
      }
      ...
      if (selectedDirectoryPath.value === node.path) {
        ...
        selectedDirectoryPath.value = newPath + selectedDirectoryPath.value.slice(node.path!.length)
      }
      ElMessage.success('分组已重命名')
      await refreshAll()
      await nextTick()
      if (directoryTreeRef.value) directoryTreeRef.value.setCurrentKey(`dir-${newPath}`)
    }
  } catch (error: any) {
    if (error !== 'cancel') ElMessage.error(error.message || error.response?.data?.detail || '重命名分组失败')
  }
}

// 删除目录——后端统一删除目录记录和该目录下的所有用例
const handleDeleteDirectory = async (node: DirectoryTreeNode) => {
  if (!node.path) {
    ElMessage.warning('不能删除根分组')
    return
  }
  const caseCount = treeTestcases.value.filter(tc => {
    const m = tc.module || ''
    return m === node.path || m.startsWith(`${node.path}/`)
  }).length
  try {
    await ElMessageBox.confirm(
      caseCount > 0
        ? `确定要删除分组"${node.label}"吗？这将同时删除该分组下的 ${caseCount} 个用例。`
        : `确定要删除空分组"${node.label}"吗？`,
      '删除分组',
      { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }
    )
    const dirRecord = treeDirectories.value.find(d => d.path === node.path)
    if (dirRecord) {
      // 有目录记录：后端接口会级联删除子目录和用例
      await testcaseApi.deleteTestCaseDirectory(dirRecord.id)
    } else {
      // 纯用例 module 推导的目录：逐条删除用例
      const affected = treeTestcases.value.filter(tc => {
        const m = tc.module || ''
        return m === node.path || m.startsWith(`${node.path}/`)
      })
      for (const tc of affected) {
        if (tc.id) await testcaseApi.deleteTestCase(tc.id)
      }
    }
    if (selectedDirectoryPath.value === node.path || selectedDirectoryPath.value?.startsWith(`${node.path}/`)) {
...
const handleExport = async (format: 'xlsx' | 'csv' = 'xlsx') => {
  ...
  try {
    ...
    if (selectedDirectoryPath.value !== null) params.module = selectedDirectoryPath.value
    if (testcaseSearchKeyword.value) params.search = testcaseSearchKeyword.value
    const response = await testcaseApi.exportTestCases(params)
    ...
    link.download = `testcases-${dateStr}.${format}`
...
const handleImportUpload = async () => {
  ...
  const projectId = getProjectId()
  if (!projectId) {
    ElMessage.warning('请先在顶部选择项目，再导入用例')
    return
  }
  importUploading.value = true
  try {
    const result = await testcaseApi.importTestCases(projectId, importFile.value)
    importResult.value = result
    if (result.imported > 0) {
      ElMessage.success(result.message || '导入完成')
      refreshAll()
    } else {
      ElMessage.warning(result.message || '未导入任何用例')
    }
  } catch (error: any) {
    ElMessage.error(error.message || error.response?.data?.detail || '导入失败')
  } finally {
...
const handleBatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定删除选中的 ${selectedTestCases.value.length} 个用例吗？该操作不可恢复。`,
      '提示',
      { type: 'warning' }
    )
    ...
    if (!ids.length) {
      ElMessage.warning('未获取到有效的用例 ID')
      return
    }
    await testcaseApi.batchDeleteTestCases(ids)
    ElMessage.success('批量删除成功')
    selectedTestCases.value = []
    refreshAll()
  } catch (error: any) {
    if (error !== 'cancel') {
      ElMessage.error(error.message || error.response?.data?.detail || '批量删除失败')
    }
  }
}
...
const cleanupTestCasesListener = onProjectChanged(() => {
  if (!isMounted) return
  if (hasProjectSelected.value && getCurrentProjectId.value) {
    filters.value.project_id = getCurrentProjectId.value
  }
