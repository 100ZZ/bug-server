    ...
    <el-card class="table-card">
      ...
      <div style="margin-top: 16px; text-align: right;">
        <el-pagination
          ...
          @size-change="() => { currentPage.value = 1; loadTasks() }"
        />
      </div>
    </el-card>
...
const loadAvailableApis = async () => {
  try {
    const response = await getApiEndpoints({ project_id: formData.value.project_id, page: 1, page_size: 1000 })
    const data = Array.isArray(response) ? response[0] : (response.data || response)
    availableApis.value = data?.items || []
...
const loadAvailableFlows = async () => {
  try {
    const response = await getApiFlows({ project_id: formData.value.project_id, page: 1, page_size: 1000 })
    const data = Array.isArray(response) ? response[0] : (response.data || response)
    availableFlows.value = data?.items || []
