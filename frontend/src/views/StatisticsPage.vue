...
const loadAllStatistics = async () => {
  ...
  try {
    ...
    // 加载测试用例统计
    testCasesData.value = (await testCaseApi.getTestCases({ project_id: projectId, page_size: 10000 })).items
    overview.value.testCases = testCasesData.value.length
    
    // 加载接口统计
    apiEndpointsData.value = (await apiTestApi.getApiEndpoints({ project_id: projectId, page_size: 1000 })).items
    overview.value.apiEndpoints = apiEndpointsData.value.length
    
    // 加载流程统计
    apiFlowsData.value = (await apiTestApi.getApiFlows({ project_id: projectId, page_size: 1000 })).items
    overview.value.apiFlows = apiFlowsData.value.length
    
    // 加载测试任务统计
    testTasksData.value = (await apiTestApi.getTestTasks({ project_id: projectId, page_size: 1000 })).items
