  ...
  <div class="api-environment-page">
    ...
    <el-card class="table-card">
      ...
      >
        ...
        <el-table-column prop="description" label="配置说明" align="center" show-overflow-tooltip />
        <el-table-column prop="base_url" label="配置信息" align="center" show-overflow-tooltip />
    ...
    <el-dialog v-model="dialogVisible" width="700px" :close-on-click-modal="true">
      <template #header>
        <div class="dialog-header">
          ...
          <span class="dialog-description">{{ dialogTitle === '新建配置' ? '创建新的配置项' : '修改配置信息' }}</span>
        </div>
      </template>
      <el-form :model="formData" label-width="100px">
        ...
        <el-form-item label="配置说明" required>
          <el-input v-model="formData.description" type="textarea" :rows="3" placeholder="请输入配置说明" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const handleSave = async () => {
  if (!formData.project_id || !formData.name || !formData.base_url || !formData.description) {
...
const cleanupEnvListener = onProjectChanged(() => {
  if (hasProjectSelected.value && getCurrentProjectId.value) {
    filters.project_id = getCurrentProjectId.value
  }
  loadProjects()
