    ...
    <el-card class="table-card">
      ...
      <div style="margin-top: 16px; text-align: right;">
        <el-pagination
          ...
          @size-change="() => { currentPage.value = 1; loadFlows() }"
        />
      </div>
    </el-card>
      ...
      <div class="flow-editor-container">
        ...
        <div class="flow-execution-chain">
          ...
          <div v-if="flowEndpointKeyword.trim()" class="endpoint-search-results">
            <div v-if="endpointSearchLoading" style="padding: 8px 12px; color: #909399; font-size: 13px;">搜索中...</div>
            <div v-else-if="filteredFlowEndpoints.length === 0" style="padding: 8px 12px; color: #909399; font-size: 13px;">暂无匹配接口</div>
            <div
...
const paginatedFlows = computed(() => flows.value)
...
const endpointSearchResults = ref<ApiEndpoint[]>([])
const endpointSearchLoading = ref(false)
...
const setLocalVariables = (vars: Array<{ id?: number; key?: string; value?: string }> | null | undefined) => {
  if (!vars || vars.length === 0) {
    // 没有变量时，保持与新建时一致：展示一行空变量
    localVariables.value = [{ key: '', value: '' }]
  } else {
    localVariables.value = vars.map(v => ({
      id: v.id,
      key: v.key || '',
      value: v.value || ''
    }))
...
const filteredFlowEndpoints = computed(() => endpointSearchResults.value)

const doEndpointSearch = () => {
  const keyword = flowEndpointKeyword.value.trim()
  if (!keyword || !flowForm.project_id) {
    endpointSearchResults.value = []
    return
  }
  endpointSearchLoading.value = true
  apitestApi
    .getApiEndpoints({ project_id: flowForm.project_id, keyword, page_size: 50 })
    .then(res => { endpointSearchResults.value = res.items })
    .catch(() => { endpointSearchResults.value = [] })
    .finally(() => { endpointSearchLoading.value = false })
}

watch(flowEndpointKeyword, () => {
  if (endpointSearchTimer) clearTimeout(endpointSearchTimer)
  endpointSearchTimer = setTimeout(doEndpointSearch, 300)
})
...
const loadFlows = async () => {
  ...
  try {
    ...
    const res = await apitestApi.getApiFlows(params)
    flows.value = res.items
    totalFlows.value = res.total
  } catch (error: any) {
...
const loadFlowVariables = async (flowId: number) => {
  try {
    ...
    if (row?.global_variables) {
      const vars = Object.entries(row.global_variables).map(([key, value]) => ({
        key,
        value: String(value)
      }))
...
const handleCreateFlow = () => {
  ...
  if (flowForm.project_id) {
    apitestApi
      .getApiEndpoints({ project_id: flowForm.project_id, page_size: 500 })
      .then(res => {
        flowEndpoints.value = res.items
  ...
  if (flowForm.steps.length > 0) {
    ...
    if (stepEndpointIds.length > 0) {
      try {
        if (flowForm.project_id) {
          try {
            const res = await apitestApi.getApiEndpoints({ project_id: flowForm.project_id, page_size: 500 })
            flowEndpoints.value = res.items
            endpoints.value = res.items
          } catch (error) {
            // 忽略错误
        ...
        const allRes = await apitestApi.getApiEndpoints({ page_size: 1000 })
        const missingIds = stepEndpointIds.filter(id => !endpoints.value.find(ep => ep.id === id))
        if (missingIds.length > 0) {
          const missingEndpoints = allRes.items.filter(ep => missingIds.includes(ep.id))
...
const onProjectChange = async () => {
  ...
  if (flowForm.project_id) {
    ...
    if (stepEndpointIds.length > 0) {
      try {
        const allRes = await apitestApi.getApiEndpoints({ page_size: 1000 })
        const neededEndpoints = allRes.items.filter(ep => stepEndpointIds.includes(ep.id))
...
const showFlowEndpoints = async (flow: ApiTestFlow) => {
  ...
  if (endpointList.length < endpointIds.length) {
    try {
      const allRes = await apitestApi.getApiEndpoints({ page_size: 1000 })
      endpointList = allRes.items.filter(ep => endpointIds.includes(ep.id))
    } catch (error: any) {
...
// 监听项目切换事件（在顶层注册，避免 async onMounted 的 await 后无法注册生命周期钩子）
const cleanupProjectListener = onProjectChanged(() => {
  if (hasProjectSelected.value && getCurrentProjectId.value) {
    flowFilters.project_id = getCurrentProjectId.value
  }
  currentPage.value = 1
