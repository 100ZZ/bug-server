        ...
        <el-table-column label="收藏" width="80" align="center">
          ...
          <div style="margin-top: 12px; text-align: right;">
            <el-pagination
              ...
              @size-change="() => { pagination.currentPage = 1; loadEndpoints() }"
            />
          </div>
        </el-card>
        ...
        <el-tab-pane label="从URL同步" name="url">
          <el-form :model="syncFormData" label-width="120px" style="margin-top: 20px">
            ...
            <el-form-item label="Swagger路径">
              ...
              <div
                class="swagger-path-hint"
                style="font-size: 12px; color: var(--el-text-color-secondary); margin-top: 6px; line-height: 1.5"
              >
                优先请求该路径；若 404 或响应不是 OpenAPI/Swagger JSON，会自动依次尝试 /v3/api-docs、/openapi.json、/v2/api-docs（兼容 Springdoc 与 FastAPI）。
              </div>
            </el-form-item>
...
const pagedEndpoints = computed(() => endpoints.value)
...
const loadEndpoints = async () => {
  ...
  try {
    ...
    const res = await apitestApi.getApiEndpoints(params)
    endpoints.value = res.items
    totalEndpoints.value = res.total
  } catch (error: any) {
...
const cleanupApiTestListener = onProjectChanged(() => {
  if (hasProjectSelected.value && getCurrentProjectId.value) {
    filters.project_id = getCurrentProjectId.value
  }
  pagination.currentPage = 1
