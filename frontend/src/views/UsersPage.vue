  ...
  <div class="users-page">
    ...
    <el-card class="filter-card">
      ...
      <div class="filter-row">
        ...
        <el-button @click="() => { currentPage = 1; loadUsers() }">搜索</el-button>
    ...
    <el-card class="table-card">
      <el-table :data="paginatedUsers" v-loading="loading" :max-height="600">
        <el-table-column label="编号" width="80" type="index" :index="(index: number) => (currentPage - 1) * pageSize + index + 1" align="center" />
      ...
      <div style="margin-top: 12px; text-align: right;">
        <el-pagination
          ...
          @size-change="() => { currentPage = 1; loadUsers() }"
        />
      </div>
    </el-card>
...
const loadUsers = async () => {
  loading.value = true
  try {
    const res = await userApi.getUsers({ keyword: searchKeyword.value || undefined, page: currentPage.value, page_size: pageSize.value })
...
function getApiErrorMessage(error: any, defaultMsg: string): string {
  const detail = error?.response?.data?.detail
  if (Array.isArray(detail)) {
    return detail.map((item: any) => item.msg || (item.loc ? `${item.loc.join('.')}: ${item.msg}` : '')).filter(Boolean).join('；') || defaultMsg
  }
...
const handleReset = () => {
  searchKeyword.value = ''
  currentPage.value = 1
  loadUsers() // currentPage.value is valid in script
}
