...
// 接口端点管理
export const getApiEndpoints = (params?: { project_id?: number; method?: string; tag?: string; keyword?: string; is_favorite?: boolean; page?: number; page_size?: number }) => {
  return request.get<any, { total: number; items: ApiEndpoint[]; page: number; page_size: number }>('/api-endpoints', { params })
}
...
// 流程测试
export const getApiFlows = (params?: { project_id?: number; keyword?: string; is_favorite?: boolean; page?: number; page_size?: number }) => {
  return request.get<any, { total: number; items: ApiTestFlow[]; page: number; page_size: number }>('/api-flows', { params })
}
...
export const getTestTasks = (params?: {
  ...
  return request.get<any, { total: number; items: TestTask[]; page: number; page_size: number }>('/test-tasks', { params })
}
